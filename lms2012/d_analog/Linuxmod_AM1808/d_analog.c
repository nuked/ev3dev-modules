 ...
 * Modified for Ralph Hempel's ev3dev infrastructure, 2014 by Fred Barnes <frmb@kent.ac.uk>.
 *
...
/*{{{  driver/device constants*/
#define MODULE_NAME	"ev3dev_analog"

#define DEVICE1_NAME	"ev3dev_analog"
#define DEVICE2_NAME	"ev3dev_tst_pin"
#define DEVICE3_NAME	"ev3dev_dcm"

/*}}}*/
/*{{{  some globals/externs for the ev3dev platform*/
...
*/
/*}}}*/
/*{{{  various input device-specific configuration constants*/
...
*/
/*}}}*/
/*{{{  various output device-specific configuration constants*/
...
/*}}}*/
/*{{{  define port counts (inputs and outputs)*/
...
/*{{{  UBYTE InputReadMap[INPUTADC]: physical-to-logical ADC channel mapping*/
...
static const int legoev3_iport_gpio[][INPUT_PORT_PINS] = {
	{				/* input port 1 */
		GPIO_TO_PIN(8, 10),	/* pin 1: I_ONA		- 9v enable (high) */
		GPIO_TO_PIN(2, 2),	/* pin 2: LEGDETA	- digital input puller up */
		GPIO_TO_PIN(0, 2),	/* pin 5: DIGIA0	- digital input/output */
		GPIO_TO_PIN(0, 15),	/* pin 6: DIGIA1	- digital input/output */
		GPIO_TO_PIN(8, 11)	/* buffer disable */
	},
	{				/* input port 2 */
		GPIO_TO_PIN(8, 12),	/* pin 1: I_ONB		- 9v enable (high) */
		GPIO_TO_PIN(8, 15),	/* pin 2: LEGDETB	- digital input puller up */
		GPIO_TO_PIN(0, 14),	/* pin 5: DIGIB0	- digital input/output */
		GPIO_TO_PIN(0, 13),	/* pin 6: DIGIB1	- digital input/output */
		GPIO_TO_PIN(8, 14)	/* buffer disable */
	},
	{				/* input port 3 */
		GPIO_TO_PIN(8, 9),	/* pin 1: I_ONC		- 9v enable (high) */
		GPIO_TO_PIN(7, 11),	/* pin 2: LEGDETC	- digital input puller up */
		GPIO_TO_PIN(0, 12),	/* pin 5: DIGIC0	- digital input/output */
		GPIO_TO_PIN(1, 14),	/* pin 6: DIGIC1	- digital input/output */
		GPIO_TO_PIN(7, 9)	/* buffer disable */
	},
	{				/* input port 4 */
		GPIO_TO_PIN(6, 4),	/* pin 1: I_OND		- 9v enable (high) */
		GPIO_TO_PIN(7, 8),	/* pin 2: LEGDETD	- digital input puller up */
		GPIO_TO_PIN(1, 13),	/* pin 5: DIGID0	- digital input/output */
		GPIO_TO_PIN(1, 15),	/* pin 6: DIGID1	- digital input/output */
		GPIO_TO_PIN(7, 10)	/* buffer disable */
	}
};

static const int legoev3_oport_gpio[][OUTPUT_PORT_PINS] = {
	{				/* output port 1 */
		GPIO_TO_PIN(0, 3),	/* pin 1: MAIN0 */
		GPIO_TO_PIN(4, 12),	/* pin 2: MAIN1 */
		GPIO_TO_PIN(5, 4),	/* pin 5: DETA0 TP18 */
		GPIO_TO_PIN(5, 11),	/* pin 5: INTA0 */
		GPIO_TO_PIN(0, 4)	/* pin 6: DIRA */
	},
	{				/* output port 2 */
		GPIO_TO_PIN(3, 15),	/* pin 1: MBIN0 */
		GPIO_TO_PIN(3, 6),	/* pin 2: MBIN1 */
		GPIO_TO_PIN(2, 5),	/* pin 5: DETB0 TP19 */
		GPIO_TO_PIN(5, 8),	/* pin 5: INTB0 */
		GPIO_TO_PIN(2, 9)	/* pin 6: DIRB */
	},
	{				/* output port 3 */
		GPIO_TO_PIN(5, 10),	/* pin 1: MCIN0 */
		GPIO_TO_PIN(5, 3),	/* pin 2: MCIN1 */
		GPIO_TO_PIN(3, 2),	/* pin 5: DETC0 TP20 */
		GPIO_TO_PIN(5, 13),	/* pin 5: INTC0 */
		GPIO_TO_PIN(3, 14)	/* pin 6: DIRC */
	},
	{				/* output port 4 */
		GPIO_TO_PIN(6, 8),	/* pin 1: MDIN0 */
		GPIO_TO_PIN(5, 9),	/* pin 2: MDIN1 */
		GPIO_TO_PIN(5, 15),	/* pin 5: DETD0 TP21 */
		GPIO_TO_PIN(6, 9),	/* pin 5: INTD0 */
		GPIO_TO_PIN(2, 8)	/* pin 6: DIRD */
	}
};
...
/*{{{  global variables for device 1 (analog ports)*/
...
/*{{{  global variables for device 1 (old NXT colour sensor)*/
...
/*{{{  global variables for device 1 (not supporting old NXT colour sensor)*/
...
/*}}}*/
/*{{{  DcmStateText (char [][50]): constant state strings*/
...
/*{{{  static void input_port_float (int port)*/
/*
 *	floats a particular input port's pins.
 */
static void input_port_float (int port)
{
...
/*}}}*/
/*{{{  static void output_port_float (int port)*/
/*
 *	floats a particular output port's pins.
 */
static void output_port_float (int port)
{
...
/*{{{  constants for device 2 (test pin)*/
...
/*{{{  NXT colour sensor state and related*/
			...
			for (Port = 0; Port < NO_OF_INPUT_PORTS; Port++) {	// look at one port at a time
				...
				case DCM_FLOATING:
					{	// pins floating - check and for connection event
						...
						Event = 0;
						if (!(PINRead (Port, INPUT_PORT_PIN2))) {	// pin 2 low

							Event |= (0x01 << INPUT_PORT_PIN2);
						}
						if ((*pAnalog).InPin1[Port] < VtoC (IN1_NEAR_5V)) {	// pin 1 loaded
				...
				case DCM_CONNECTION:
					{	// something is connected - try to evaluate
						...
						if (InputPort[Port].Event & (0x01 << INPUT_PORT_PIN2)) {	// pin 2 is low

							InputPort[Port].State = DCM_PIN2_LOW;
						} else {
							if (InputPort[Port].Event & (0x01 << INPUT_PORT_VALUE)) {	// pin 1 is loaded

								InputPort[Port].State = DCM_PIN1_LOADED;
							} else {
								if (InputPort[Port].Event & (0x01 << INPUT_PORT_PIN6)) {	// pin 6 is high

									InputPort[Port].State = DCM_PIN6_HIGH;
								} else {
									if (InputPort[Port].Event & (0x01 << INPUT_PORT_PIN6)) {	// pin 5 is low
				...
				case DCM_CONNECTION:
					{	// something is connected - try to evaluate
						...
						if (OutputPort[Port].Connected) {
							printk ("\r\no    %d Type = %c, Float = %u, Low = %u\r\n", Port, (char) (*pAnalog).OutDcm[Port],
								(unsigned int) OutputPort[Port].Value5Float, (unsigned int) OutputPort[Port].Value5Low);
						}
#endif
					}
					break;
...
/*{{{  static int analog_module_init (void)*/
/*
 *	initialises the analog module and each of the sub-devices.
...
/*}}}*/
/*{{{  static void analog_module_exit (void)*/
/*
 *	shuts-down the various analog module components.
 */
static void analog_module_exit (void)
{
#ifdef DEBUG
...
/*{{{  static int ev3dev_analog_init (void)*/
/*
 *	initialises the analog module, connects up to ev3dev infrastructure.
...
static int ev3dev_analog_init (void)
{
	...
	if (!ev3dev) {
		printk (KERN_ERR MODULE_NAME ": ev3dev platform device not yet registered, load the ev3dev module\n");
		ret = -ENODEV;
		goto out_err0;
	}

	analog = platform_device_alloc ("analog", -1);
	if (!analog) {
		printk (KERN_ERR MODULE_NAME ": failed to allocate ev3dev/analog device\n");
		ret = -ENOMEM;
		goto out_err1;
	}

#if 1
	printk (KERN_INFO MODULE_NAME ": allocated platform device at 0x%8.8x\n", (unsigned int)analog);
#endif

	/* connect */
	analog->dev.parent = &ev3dev->dev;

	ret = platform_device_add (analog);
	if (ret) {
		printk (KERN_ERR MODULE_NAME ": failed to register device, error %d\n", ret);
		goto out_err2;
	}

	ret = analog_module_init ();
	if (ret) {
		printk (KERN_ERR MODULE_NAME ": failed to initialise analog driver, error %d\n", ret);
		goto out_err3;
	}
...
out_err3:
	platform_device_del (analog);
out_err2:
	platform_device_put (analog);
	analog = NULL;
out_err1:
out_err0:
	return ret;
...
/*}}}*/
/*{{{  static void ev3dev_analog_exit (void)*/
/*
 *	shuts-down the analog module, disconnects from ev3dev infrastructure.
 */
static void ev3dev_analog_exit (void)
{
	analog_module_exit ();
...
module_init (ev3dev_analog_init);
module_exit (ev3dev_analog_exit);

MODULE_LICENSE ("GPL");
MODULE_AUTHOR ("The LEGO Group and Fred Barnes <frmb@kent.ac.uk>");
MODULE_DESCRIPTION ("LEGO EV3 driver for analog devices, the test pin and the device connection manager");
